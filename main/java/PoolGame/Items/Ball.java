...
public class Ball implements Drawable, Movable {
    ...
    public Ball(Ball ball) {
        this.shape = new Circle(ball.originalPos[0], ball.originalPos[1], RADIUS);
        this.mouseDragLine = new Line();
        this.mouseDragLine.setVisible(false);
        this.setColour((Color) ball.getColor());
        this.setXPos(ball.getXPos());
        this.setYPos(ball.getYPos());
        this.setXVel(ball.getXVel());
        this.setYVel(ball.getYVel());
        this.vel[0] = ball.getXVel();
        this.vel[1] = ball.getYVel();
        this.setDisabled(ball.isDisabled());
        this.setMass(ball.mass);
        this.setBallType(ball.getBallType());
        this.setPocketAction(ball.pocketAction);
    }

    public int getValue() {
        if(colour==Color.valueOf("red"))
            return 1;
        if(colour==Color.valueOf("yellow"))
            return 2;
        if(colour==Color.valueOf("green"))
            return 3;
        if(colour==Color.valueOf("brown"))
            return 4;
        if(colour==Color.valueOf("blue"))
            return 5;
        if(colour==Color.valueOf("purple"))
            return 6;
        if(colour==Color.valueOf("black"))
            return 7;
        if(colour==Color.valueOf("orange"))
            return 8;
        return 1;
    }
    ...
    public void setDisabled(boolean disabled) {
        this.shape.setVisible(!disabled);
    ...
    public boolean isCueBallHit() {
        return isCueBallHit;
    }

    public void setCueBallHit(boolean cueBallHit) {
        isCueBallHit = cueBallHit;
    }

    private boolean isCueBallHit=false;
    /**
